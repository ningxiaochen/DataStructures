...
"""
# 第一种  穷举法：排除。原因：如果字符串过长，20个长度  20！


# 第二种  检查  第一个字符串是不是出现在第二个字符串中  o(n^2)
def solutions(s1, s2):
    alist = list(s2)
    pos1 = 0
    flag = True
...
print(solutions('abcd','dbca'))


# 第三种   技术和比较法
# 计算每个字符出现的次数
# 复杂度 T（n）=n+n+26 = 2n+26  o(n)
...
# 第四种  排序比较  ： s1,s2 他们都是由完全相同的字符串组成的。
# 我们可以按照从a-z 排列每一个字符串，如果两个字符串相同，那这两个字符串就是乱序字符串
# 复杂度 ： O(n^2)
def solutions3(s1, s2):
    alist1 = list(s1)
    alist2 = list(s2)
    # 排序  复杂度O（n^2）或者O(logn)
    alist1.sort()
    alist2.sort()
